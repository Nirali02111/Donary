...
export class AdvancereceiptActionPopupComponent implements OnInit {
  ...
  @Input() set List(datalist) {
    ...
    this.commonAPIMethodService.getLabelText(eventGuId).subscribe((res: any) => {
      ...
      this.allPhoneLabelsArray = this.allPhoneLabelsArray.filter(x => x.labelType == "Phone");
      this.allPhoneLabelsArray = this.allPhoneLabelsArray.map(item => {
        item.isLabelNameChecked = false
        return item;
      });
      this.allEmailLabelsArray = this.allEmailLabelsArray.filter(x => x.labelType == "Email");
      this.allEmailLabelsArray = this.allEmailLabelsArray.map(item => {
        item.isLabelNameChecked = false
        return item;
      });
      this.allAddressLabelsArray = res.filter(x => x.labelType == "Address");
      ...
      datalist = datalist.map((x, index) => {
        ...
        if (x.phoneLabels && x.phoneNumbers && x.smsSent) {
          ...
          for (let i = 0; i < keys.length; i++) {
            const found = this.allPhoneLabelsArray.some(el => el.labelName.toLowerCase() == keys[i].toLowerCase());
            if (found) {
              obj.push({
                "labelID": index + '_' + i,
                "labelType": "Phone",
        ...
        if (x.emailLabels && x.emailAddresses) {
          ...
          this.allEmailLabelsArray.map(item => {
            const found = emailObj.some(el => el.labelName.toLowerCase() == item.labelName.toLowerCase());
            if (!found)
              emailObj.push({
        ...
        const addressObj = this.addLabels(addressLabels, addresses, "Address", index, addressId);
        const addressLabelArray = this.addLabelRemoveDuplicate(this.allAddressLabelsArray, addressObj);
        return {
          ...x,
          addressLabelArray
        }
 ...
 @Input() pageRoot:string
  ...
  this.commonAPIMethodService.getLabelText(eventGuId).subscribe((res: any) => {
    ...
    this.allPhoneLabelsArray = this.allPhoneLabelsArray.filter(x => x.labelType == "Phone");
        this.allPhoneLabelsArray = this.allPhoneLabelsArray.map(item=>{
          item.isLabelNameChecked=this.labelNameShowHideArray.includes(item.labelName)
          return item;
        });
        this.allEmailLabelsArray = this.allEmailLabelsArray.filter(x => x.labelType == "Email");
        this.allEmailLabelsArray = this.allEmailLabelsArray.map(item=>{
          item.isLabelNameChecked=this.emailLabelNameShowHideArray.includes(item.labelName)
          return item;
        });
    ...
    let datalist=listMailData;
     datalist = datalist.map((x, index) => {
      ...
      this.originalList=datalist;
      //update checkbox
      this.selectedAccountList=this.selectedAccountList.filter(item=>{
      ...
      this.dataList.map(item => {
        if (item.phoneLabelArray && item.phoneLabelArray.length > 0) {
          item.phoneLabelArray.map(x => {
              ...
              if(x.phoneValue){
              this.selectedAccountList.push({accountId: item.accountId,paymentId: item.paymentId,phoneNumber: x.phoneValue,labelName:x.labelName,labelType:x.labelType});
              }
            }
          })
        }
        if (item.emailLabelArray && item.emailLabelArray.length > 0) {
          item.emailLabelArray.map(x => {
              ...
              if(x.emailValue){
              this.selectedAccountList.push({accountId: item.accountId,paymentId: item.paymentId,email: x.emailValue,labelName:x.labelName,labelType:x.labelType});
              }
              }
          })
        }
  ...
  addLabels(itemLabels = [], valuelList = [], labelType = "",i="",addressId=null) {
    let resultValue = [];
    for (let index = 0; index < itemLabels.length; index++) {
      ...
      if (label && labelType!="Address") {
         obj={
          ...
          obj=this.createAddressObj(labelId,labelType,label,value,addressIdValue);
        }
  ...
  addLabelRemoveDuplicate(allLabel = [], objArray = []) {
    ...
    allLabel.map(item => {
      const found = objArray.some(el => el.labelName.toLowerCase() == item.labelName.toLowerCase());
      let resValue=objArray.filter(x=>x.labelName.toLowerCase() == item.labelName.toLowerCase()).map(x=>x.addressId).toString();
      if (!found) {
        if(item.labelType=="Address")
        {
          let res=this.createAddressObj(item.labelID,item.labelType,item.labelName,"",resValue)
    ...
    for (var i = 0; i < this.dataList.length; i++) {
      if(this.dataList[i].defaultPhone){
        this.dataList[i].phoneNumber = this.dataList[i].defaultPhone;
          ...
          if ( this.dataList[i].phoneNumbers &&
              this.dataList[i].phoneNumbers.indexOf(",") > -1
               ...
               const obj = keys.reduce((obj, key, index) => ({ ...obj, [key]: values[index] }), {});
               this.dataList[i].phoneNumber = obj[1]==undefined?obj[0]:obj[1];
              this.dataList[i].phoneNumberListArray = phoneNumberListArray.filter(item => item.phoneNumber != this.dataList[i].phoneNumber);
            } else {
        ...
        if(this.dataList[i].defaultEmail){
          this.dataList[i].email = this.dataList[i].defaultEmail;
            ...
            if ( this.dataList[i].emailAddresses &&
  ...
  openPaymentCardPopup(paymentId) {
    if (paymentId != null && paymentId != 0) {
      ...
      const modalRef = this.commonMethodService.openPopup(PaymentCardPopupComponent, this.modalOptions);
      var objDonorCard = {
  ...
  totalSelectedShow() {
    ...
    const dd = _(this.selectedAccountList).chain().groupBy(p => p.accountId)
      .map(props => ({
        ..._.head(props),
      })).value()
    return dd.length;
  ...
  findAndSelectForDonorName(found) {
    if (!this.checkAccountIsSelectOrNot(found.paymentId)) {
      let objIndex = this.selectedAccountList.findIndex((obj => obj.paymentId === found.paymentId));
      if (objIndex !== -1) {
  ...
  selectName(event, type, paymentId) {
    if (type == "selectAll") {
      ...
      if (event.target.checked) {
        ...
        this.selectedAccountList = this.selectedAccountList.filter(o => {o.paymentId !== paymentId})
      }
    }
  }
  ...
  selectPhoneNumber(event, type, paymentId) {
    if (type == "selectAll") {
      if (event.target.checked) {
        this.dataList.map(o => {
          if (o.phoneNumber) {
            let objIndex = this.selectedAccountList.findIndex((obj => obj.paymentId === o.paymentId));
            if (objIndex !== -1) {
  ...
  selectAddressLabels(event, labelName,i,labelType) {
    ...
    if (event.target.checked) {
      ...
      for (let index = 0; index < this.dataList.length; index++) {
        ...
        const isValid=item.validation==undefined?true:item.validation.isValid?true:false;
        this.selectAllLabels(item.addressLabelArray,labelName,labelType,item.accountId,item.paymentId,isValid);
      }
      return false;
    }
  ...
  isAddressLabelNameShowHide(labelName){
    return this.AddressLabelNameShowHideArray && this.AddressLabelNameShowHideArray.length>0 && labelName && this.AddressLabelNameShowHideArray.includes(labelName);
  }
 

  isAddressLabelChecked(paymentId, accountId, selectedAddress, labelName, labelType) {

    const found = this.selectedAccountList.some(el => el.labelName == labelName && el.paymentId == paymentId && el.accountId == accountId && el.address == selectedAddress && el.labelType == labelType);
    return found;
...
deSelectLabel(event, paymentId, accountId, labelValue, labelName, labelType) {
  ...
  if (event.target.checked) {
    ...
    const obj = this.getParameters(accountId, paymentId, labelValue, labelName, labelType);
  ...
  this.selectedAccountList = this.selectedAccountList.filter(e => !(e.accountId == accountId && e.labelName == labelName && e.labelType == labelType));

}
  selectAllLabels(lableArray = [], labelName = "", labelType = "", accountId,paymentId,isValid=true) {
    if (lableArray && lableArray.length > 0) {
      for (let index = 0; index < lableArray.length; index++) {
        ...
        if (element.labelName.toLowerCase() == labelName.toLowerCase() && labelValue && isValid) {
          const obj = this.getParameters(accountId,paymentId, labelValue, labelName, labelType,addressId);
            this.selectedAccountList.push(obj);
        }
      }
    }
    ...
    if (type == "selectAll") {
      if (event.target.checked) {
        this.dataList.map(o => {
          if (o.defaultAddress) {
            let objIndex = this.selectedAccountList.findIndex((obj => obj.paymentId === o.paymentId));
            if (objIndex !== -1) {
              this.selectedAccountList[objIndex].defaultAddress = o.defaultAddress;
            } else {
              this.selectedAccountList.push({
                accountId: o.accountId,
                paymentId: o.paymentId,
      ...
      if (event.target.checked) {
        ...
        if (found.defaultAddress) {
          let objIndex = this.selectedAccountList.findIndex((obj => obj.paymentId === found.paymentId));
          if (objIndex !== -1) {
            this.selectedAccountList[objIndex].defaultAddress = found.defaultAddress;
          } else {
            this.selectedAccountList.push({
              accountId: found.accountId,
              paymentId: found.paymentId,
  ...
  getParameters(accountId,paymentId, labelValue, labelName, labelType,addressId=null) {
    let obj: SelectedDonorObj = {accountId:accountId, paymentId: paymentId, labelName: labelName, labelType: labelType };
    obj = labelType == "Phone" ? { ...obj, phoneNumber: labelValue } : labelType == "Email" ? { ...obj, email: labelValue } : labelType == "Address" ? { ...obj, address: labelValue,addressId:addressId } : obj
    return obj;
  }
 ...
 //this.isloading=true
    ...
    const allValidationReq = _.chunk(this.originalList, 5).map((grpChunk) => {
      const grpChunkxmlsList = grpChunk.map((o) => {
       ...
       if(o.addressLabels && o.addressLabels.indexOf("Home")> -1 && this.selectedAccountList.find(x=>x.accountId== o.accountId && x.labelType=="Address")){
      ...
      forkJoin(allValidationReq).subscribe((results) => {
        ...
        this.dataList = this.originalList.map((o) => {
          ...
          if (findResponse) {
            if(!isError){
            isError=findResponse && findResponse.isValid==true?false:true;
            }
            if(!findResponse.isValid)
            {
              this.selectedAccountList=this.selectedAccountList.filter(x=>x.accountId!=o.accountId)
            }
            return {
              ...o,
  ...
  checkEmailIsSelectOrNot(accId, email){
    const found = this.selectedAccountList.find(e => e.paymentId === accId &&  e.email && e.email === email);
    return found ? true : false;
  }

  checkPhoneIsSelectOrNot(accId, phoneNumber){
    const found = this.selectedAccountList.find(e => e.paymentId === accId && e.phoneNumber && e.phoneNumber === phoneNumber);
    return found ? true : false;
  }

  checkMailIsSelectOrNot(accId, defaultAddress){
    const found = this.selectedAccountList.find(e => e.paymentId === accId && e.defaultAddress && e.defaultAddress === defaultAddress);
    return found ? true : false;
  }

  checkPrintIsSelectOrNot(accId){
    const found = this.selectedAccountList.find(e => e.paymentId === accId && e.isPrint);
    return found ? true : false;
  }

  checkDonorIsDisable(accId) {
    const found = this.dataList.find(e => e.paymentId === accId);
    !found.email && !found.phoneNumber && (!found.defaultAddress && !found.cityStateZip)
  }
  ...
  editDonor(accountId) {
    ...
    const modalRef = this.commonMethodService.openPopup(DonorSaveComponent, this.modalOptions);
    var eventGuId = this.localstoragedataService.getLoginUserEventGuId();
    this.donorService.getDonorById(eventGuId, accountId).subscribe((res: any) => {
      this.isloading = false;
      modalRef.componentInstance.Type = "edit";
      modalRef.componentInstance.EditDonorData = res;
    });
    
    modalRef.componentInstance.emtOutputEditDonorUpdatedValue.subscribe((res: any) => {
      this.isloading = true;
        ...
        if (item.phoneLabelArray && item.phoneLabelArray.length > 0) {
          item.phoneLabelArray.map(x => {
            if (x.labelName.toLowerCase() == labelName.toLowerCase()) {
              ...
              if(x.phoneValue){
              this.selectedAccountList.push({accountId: item.accountId,paymentId: item.paymentId,phoneNumber: x.phoneValue,labelName:labelName,labelType:labelType});
              }
            }
          })
        }
  ...
  isCellLabelChecked(accountId,selectedPhone,paymentId,labelName,labelType){
    const found = this.selectedAccountList.some(el => el.labelName ==labelName && el.accountId==accountId && el.phoneNumber==selectedPhone && el.paymentId==paymentId && el.labelType==labelType);
    return found;
  }
  isEmailLabelChecked(accountId,selectedEmail,paymentId,labelName,labelType){
    const found = this.selectedAccountList.some(el => el.labelName ==labelName && el.accountId==accountId && el.email==selectedEmail && el.paymentId==paymentId && el.labelType==labelType);
    return found;
  }
  isLabelNameShowHide(labelName){
    if(this.labelNameShowHideArray && this.labelNameShowHideArray.length>0 && labelName){
    if(this.labelNameShowHideArray.includes(labelName)){
      return true;
    }
    }
  ...
  isEmailLabelNameShowHide(labelName){
    if(this.emailLabelNameShowHideArray && this.emailLabelNameShowHideArray.length>0 && labelName){
        ...
        if (item.emailLabelArray && item.emailLabelArray.length > 0) {
          item.emailLabelArray.map(x => {
            if (x.labelName.toLowerCase() == labelName.toLowerCase()) {
              ...
              if(x.emailValue){
              this.selectedAccountList.push({accountId: item.accountId,paymentId: item.paymentId,email: x.emailValue,labelName:labelName,labelType:labelType});
              }
            }
          })
        }
  ...
  deSelectCellLabel(event,accountId,selectedPhone,paymentId,labelName,labelType){
    if(event.target.checked){
      this.selectedAccountList.push({accountId:accountId,phoneNumber:selectedPhone,paymentId:paymentId,labelName:labelName,labelType:labelType})
      return false;
    }
    this.selectedAccountList = this.selectedAccountList.filter(e=>!(e.accountId==accountId && e.phoneNumber==selectedPhone && e.paymentId==paymentId && e.labelName==labelName && e.labelType == labelType));
  }
  deSelectEmailLabel(event,accountId,selectedEmail,paymentId,labelName,labelType){
    if (event.target.checked) {
      this.selectedAccountList.push({accountId:accountId,email:selectedEmail,paymentId:paymentId,labelName:labelName,labelType:labelType});
      return false;
    }
    this.selectedAccountList = this.selectedAccountList.filter(e=>!(e.accountId==accountId && e.email==selectedEmail && e.paymentId == paymentId && e.labelName == labelName && labelType == labelType));
  }
  ...
  filterBySendStatus1(isSent){
    let datalist = this.originalList.filter(item=>{
      ...
      if(this.isSentTextChecked || this.isSentMailChecked || this.isSentPrintChecked ||this.isSentEmailChecked){
        return false
      }else{
        return true;
      }

...
export class ReasonService {
   ...
   REASON_BULK_ACTIONS=`${this.version}${this.REASON_MAIN_URL}/BulkActions`;
    ...
    getReasonList(formdata: any): Observable<any> {
        return this.http.post(this.REASON_GET_LIST_URL ,formdata).pipe(response => {
          return response;
        });
      }

    getReasonCard(formdata: any): Observable<any> {
        return this.http.post(this.REASON_CARD_DATA_URL, formdata).pipe(response => {
          return response;
        });
      }
  saveReason(formdata: any): Observable<any> {
     return this.http.post(this.REASON_SAVE_DATA_URL, formdata).pipe(response => {
          return response;
        });
      }

  getReason(reasonId:number,macAddress:string,eventGuid: string,): Observable<any> {
        return this.http.get(this.REASON_GET_DATA_URL + '?reasonId=' + reasonId+'&&macAddress=' + macAddress+'&&eventGuId=' + eventGuid ).pipe(response => {
          return response;
        });
      }

  deleteReason(formdata:any) {
        return this.http.request('delete', this.REASON_DELETE_DATA_URL, { body: formdata }).pipe(response => { 
          return response;
        });    
    }
    ...
    uploadResonFile(formdata: any){
      return this.http.post(this.REASON_BULK_URL,formdata).pipe(
        respone=>{
          return respone
        }
      )
    ...
    getReasonReport(formdata: any){
      return this.http.post(this.REASON_GET_REPORT,formdata).pipe(
        respone=>{
          return respone
        }
      )
    ...
    reasonBulkAction(formdata: any){
      return this.http.post(this.REASON_BULK_ACTIONS,formdata).pipe(
        respone=>{
          return respone
        }
      )

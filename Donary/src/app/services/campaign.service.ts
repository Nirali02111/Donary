...
import { CampaignListRequestData, CampaignListResponseData, CampaignCardDataResponse } from './../models/campaign-model';
...
export class CampaignService {
    ...
    getCampaignList(formdata: CampaignListRequestData) : Observable<CampaignListResponseData>{
        return this.http.post<CampaignListResponseData>(this.GET_CAMPAIGNS_LIST_URL, formdata).pipe(response => {
            return response;
        });
    }

    getCampaignCard(formdata) : Observable<CampaignCardDataResponse> {
        return this.http.post<CampaignCardDataResponse>(this.GET_CAMPAIGNS_CARD_URL, formdata).pipe(response => {
            return response;
        });
    }
    ...
    saveCampaign(formdata: any): Observable<any> {     
      ...
      getCampaign(campaignId: number, macAddress: string, eventGuid: string): Observable<any> {     
        return this.http.get(this.GET_CAMPAIGN_URL+ '?campaignId=' + campaignId + '&&macAddress=' + macAddress + '&&eventGuid=' + eventGuid).pipe(response => {
          return response;
        });
      }
      deleteCampaign(formdata:any) {
        return this.http.request('delete', this.DELETE_CAMPAIGN_URL, { body: formdata }).pipe(response => { 
          return response;
        });    
    }
    getCampaingsReport(formdata: any): Observable<any> {     
      return this.http.post(this.GET_CAMPAINGS_REPORT_URL, formdata).pipe(response => {
        return response;
      });
    }
    ...
    //   return this.http.get(this.REASON_DOWNLOAD_TEMPLATE_URL,{ responseType: "blob" }
    //     ).pipe(
    //     response=>{
    ...
    uploadResonFile(formdata: any){
      return this.http.post(this.GET_CAMPAINGS_IMPORT_URL,formdata).pipe(
        respone=>{
          return respone
        }
      )
